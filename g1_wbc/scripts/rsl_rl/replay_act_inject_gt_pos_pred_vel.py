...
import cli_args
parser = argparse.ArgumentParser(description="Experiment 2: Inject GT hand positions + ACT predicted velocities (smoothed) to tracking policy.")
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to trajectory directory with log_dict.npy (required for GT hand positions)"
)
parser.add_argument(
    "--act_checkpoint",
    type=str,
...
parser.add_argument("--smooth_window", type=int, default=5, help="Window size for velocity smoothing")
...
class VelocitySmoother:
    ...
    def smooth(self, velocity: np.ndarray) -> np.ndarray:
...
def inject_hybrid_command(env, act_action: torch.Tensor, step_idx: int, gt_commands: dict,
                          vel_smoother: VelocitySmoother, device):
    ...
    raw_vel = act_action[6:9].cpu().numpy()
    smoothed_vel = vel_smoother.smooth(raw_vel)

    base_vel_term = cmd_manager.get_term("target_base_velocity")
    base_vel_cmd = torch.from_numpy(smoothed_vel).float().to(device)
    base_vel_term.command[:, :3] = base_vel_cmd.unsqueeze(0)

    return idx, raw_vel, smoothed_vel
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "exp2_inject_gt_pos_pred_vel"),
    ...
    print(f"[INFO] Mode: Experiment 2 - Inject GT hand positions + ACT predicted velocities (smoothed)")

    vel_smoother = VelocitySmoother(window_size=args_cli.smooth_window)
    ...
    print(f"\n[INFO] Starting Experiment 2: GT Hand Positions + ACT Predicted Velocities (Smoothed)")
    ...
    print(f"[INFO] Tracking policy receives: GT hand positions + ACT velocities (smoothed)")
    ...
    while simulation_app.is_running():
        ...
        cmd_idx, raw_vel, smoothed_vel = inject_hybrid_command(
            env, act_action, timestep, gt_commands, vel_smoother, env.unwrapped.device
        )
        ...
        if timestep % 50 == 0:
            ...
            print(f"\n[Step {timestep}] Experiment 2: GT Positions + ACT Velocities (Smoothed)")
            ...
            print(f"  Injected smoothed ACT velocity: {smoothed_vel}")
            print(f"  ACT raw velocity: {raw_vel}")
            print(f"  GT velocity: xy={gt_commands['base_vel_xy'][cmd_idx]}, yaw={gt_commands['yaw_speed'][cmd_idx]:.3f}")
            print(f"  ACT hand predictions (unused): L={act_action[0:3].cpu().numpy()}, R={act_action[3:6].cpu().numpy()}")
