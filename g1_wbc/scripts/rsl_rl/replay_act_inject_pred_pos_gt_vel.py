...
import cli_args
parser = argparse.ArgumentParser(description="Experiment 4: Inject ACT predicted hand positions + GT velocities to tracking policy.")
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to trajectory directory with log_dict.npy (required for GT velocities)"
)
parser.add_argument(
    "--act_checkpoint",
    type=str,
...
def inject_hybrid_command(env, act_action: torch.Tensor, step_idx: int, gt_commands: dict, device):
    ...
    left_hand_term.command[:, :3] = act_action[0:3].unsqueeze(0).to(device)

    right_hand_term = cmd_manager.get_term("target_right_hand_pos_in_base")
    right_hand_term.command[:, :3] = act_action[3:6].unsqueeze(0).to(device)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "exp4_inject_pred_pos_gt_vel"),
    ...
    print(f"[INFO] Mode: Experiment 4 - Inject ACT hand positions + GT velocities")
    ...
    print(f"\n[INFO] Starting Experiment 4: ACT Hand Positions + GT Velocities")
    ...
    print(f"[INFO] Tracking policy receives: ACT hand positions + GT velocities")
    ...
    while simulation_app.is_running():
        ...
        cmd_idx = inject_hybrid_command(env, act_action, timestep, gt_commands, env.unwrapped.device)
        ...
        if timestep % 50 == 0:
            ...
            print(f"\n[Step {timestep}] Experiment 4: ACT Positions + GT Velocities")
            print(f"  Injected ACT left hand: {act_action[0:3].cpu().numpy()}")
            print(f"  Injected ACT right hand: {act_action[3:6].cpu().numpy()}")
            print(f"  Injected GT velocity: xy={gt_commands['base_vel_xy'][cmd_idx]}, yaw={gt_commands['yaw_speed'][cmd_idx]:.3f}")
            print(f"  GT left hand: {gt_commands['left_hand_pos'][cmd_idx]}")
            print(f"  GT right hand: {gt_commands['right_hand_pos'][cmd_idx]}")
            print(f"  ACT velocity prediction (unused): {act_action[6:9].cpu().numpy()}")
