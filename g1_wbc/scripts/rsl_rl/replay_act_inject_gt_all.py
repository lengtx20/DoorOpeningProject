...
import cli_args
parser = argparse.ArgumentParser(description="Experiment 1: Inject ALL GT commands to tracking policy (ACT runs with sim state for monitoring).")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "exp1_inject_gt_all"),
    ...
    print(f"[INFO] Mode: Experiment 1 - Inject ALL GT commands to tracking policy")
    ...
    print(f"\n[INFO] Starting Experiment 1: All GT Actions")
    ...
    print(f"[INFO] Tracking policy receives: ALL GT (hand positions + base velocities)")
    ...
    while simulation_app.is_running():
        ...
        if timestep % 50 == 0:
            ...
            print(f"\n[Step {timestep}] Experiment 1: All GT Commands")
            ...
            print(f"  Injected GT base vel: xy={gt_commands['base_vel_xy'][cmd_idx]}, yaw={gt_commands['yaw_speed'][cmd_idx]:.3f}")
