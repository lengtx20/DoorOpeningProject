...
import cli_args
parser = argparse.ArgumentParser(description="Experiment 6: Inject ALL SMOOTHED GT commands to tracking policy (ACT runs with sim state for monitoring).")
...
parser.add_argument("--smooth_window", type=int, default=5, help="Window size for offline smoothing")
...
def load_gt_trajectory(traj_path: str, load_images: bool = False, smooth_window: int = 5):
    ...
    left_x = np.array(log_dict['left_wrist_pos_x'])
    left_y = np.array(log_dict['left_wrist_pos_y'])
    left_z = np.array(log_dict['left_wrist_pos_z'])

    right_x = np.array(log_dict['right_wrist_pos_x'])
    right_y = np.array(log_dict['right_wrist_pos_y'])
    right_z = np.array(log_dict['right_wrist_pos_z'])

    vel_x = np.array(log_dict['vel_x'])
    vel_y = np.array(log_dict['vel_y'])
    yaw_speed = np.array(log_dict['yaw_speed'])
    print(f"[INFO] Smoothing ALL GT commands with window size {smooth_window}")

    left_x_smooth = smooth_array(left_x, window_size=smooth_window)
    left_y_smooth = smooth_array(left_y, window_size=smooth_window)
    left_z_smooth = smooth_array(left_z, window_size=smooth_window)

    right_x_smooth = smooth_array(right_x, window_size=smooth_window)
    right_y_smooth = smooth_array(right_y, window_size=smooth_window)
    right_z_smooth = smooth_array(right_z, window_size=smooth_window)

    vel_x_smooth = smooth_array(vel_x, window_size=smooth_window)
    vel_y_smooth = smooth_array(vel_y, window_size=smooth_window)
    yaw_speed_smooth = smooth_array(yaw_speed, window_size=smooth_window)

    gt_commands = {
        'left_hand_pos': np.stack([left_x, left_y, left_z], axis=1),
        'right_hand_pos': np.stack([right_x, right_y, right_z], axis=1),
        'left_hand_pos_smooth': np.stack([left_x_smooth, left_y_smooth, left_z_smooth], axis=1),
        'right_hand_pos_smooth': np.stack([right_x_smooth, right_y_smooth, right_z_smooth], axis=1),
...
def inject_gt_smooth_all_command(env, step_idx: int, gt_commands: dict, device):
    ...
    left_hand_cmd = torch.from_numpy(gt_commands['left_hand_pos_smooth'][idx]).float().to(device)
    left_hand_term.command[:, :3] = left_hand_cmd.unsqueeze(0)

    right_hand_term = cmd_manager.get_term("target_right_hand_pos_in_base")
    right_hand_cmd = torch.from_numpy(gt_commands['right_hand_pos_smooth'][idx]).float().to(device)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "exp6_inject_gt_all_smooth"),
    ...
    print(f"[INFO] Mode: Experiment 6 - Inject ALL SMOOTHED GT commands to tracking policy")
    print(f"[INFO] Smoothing window size: {args_cli.smooth_window}")
    ...
    print(f"\n[INFO] Starting Experiment 6: All Smoothed GT Actions")
    ...
    print(f"[INFO] Tracking policy receives: ALL SMOOTHED GT (hand positions + base velocities)")
    ...
    while simulation_app.is_running():
        ...
        cmd_idx = inject_gt_smooth_all_command(env, timestep, gt_commands, env.unwrapped.device)
        ...
        if timestep % 50 == 0:
            ...
            print(f"\n[Step {timestep}] Experiment 6: All Smoothed GT Commands")
            print(f"  Raw GT left hand: {gt_commands['left_hand_pos'][cmd_idx]}")
            print(f"  Injected smoothed GT left hand: {gt_commands['left_hand_pos_smooth'][cmd_idx]}")
            print(f"  Raw GT right hand: {gt_commands['right_hand_pos'][cmd_idx]}")
            print(f"  Injected smoothed GT right hand: {gt_commands['right_hand_pos_smooth'][cmd_idx]}")
            print(f"  Raw GT velocity: xy={gt_commands['base_vel_xy'][cmd_idx]}, yaw={gt_commands['yaw_speed'][cmd_idx]:.3f}")
