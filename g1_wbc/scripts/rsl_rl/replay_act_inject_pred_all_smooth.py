...
import cli_args
parser = argparse.ArgumentParser(description="Experiment 7: Inject ALL ACT predictions (smoothed online) to tracking policy.")
...
parser.add_argument("--smooth_window", type=int, default=5, help="Window size for online smoothing")
...
class ActionSmoother:
    ...
    def __init__(self, window_size: int = 5, action_dim: int = 9):
...
def inject_smoothed_act_command(env, act_action: torch.Tensor, action_smoother: ActionSmoother, device):
    raw_action = act_action[0:9].cpu().numpy()

    smoothed_action = action_smoother.smooth(raw_action)

    cmd_manager = env.unwrapped.command_manager

    left_hand_term = cmd_manager.get_term("target_left_hand_pos_in_base")
    left_hand_cmd = torch.from_numpy(smoothed_action[0:3]).float().to(device)
    left_hand_term.command[:, :3] = left_hand_cmd.unsqueeze(0)

    right_hand_term = cmd_manager.get_term("target_right_hand_pos_in_base")
    right_hand_cmd = torch.from_numpy(smoothed_action[3:6]).float().to(device)
    right_hand_term.command[:, :3] = right_hand_cmd.unsqueeze(0)

    base_vel_term = cmd_manager.get_term("target_base_velocity")
    base_vel_cmd = torch.from_numpy(smoothed_action[6:9]).float().to(device)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "exp7_inject_pred_all_smooth"),
    ...
    print(f"[INFO] Mode: Experiment 7 - Inject ALL ACT predictions (smoothed online)")
    print(f"[INFO] Action smoothing window size: {args_cli.smooth_window}")

    action_smoother = ActionSmoother(window_size=args_cli.smooth_window, action_dim=9)
    ...
    print(f"\n[INFO] Starting Experiment 7: All ACT Predictions (Smoothed)")
    ...
    print(f"[INFO] Tracking policy receives: ALL ACT predictions (hand positions + velocities, smoothed online)")
    ...
    while simulation_app.is_running():
        ...
        raw_action, smoothed_action = inject_smoothed_act_command(
            env, act_action, action_smoother, env.unwrapped.device
        )
        ...
        if timestep % 50 == 0:
            ...
            print(f"\n[Step {timestep}] Experiment 7: All ACT Predictions (Smoothed)")
            print(f"  Raw ACT left hand: {raw_action[0:3]}")
            print(f"  Injected smoothed left hand: {smoothed_action[0:3]}")
            print(f"  Raw ACT right hand: {raw_action[3:6]}")
            print(f"  Injected smoothed right hand: {smoothed_action[3:6]}")
            print(f"  Raw ACT velocity: {raw_action[6:9]}")
            print(f"  Injected smoothed velocity: {smoothed_action[6:9]}")
